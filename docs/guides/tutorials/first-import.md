...
- Knowledge OS installed (`cargo install knowledge-cli`)
- A terminal with access to the `kos` CLI
...
```bash
kos search "knowledge management"
```
...
After import, the derivation layer (Layer 6) runs in the background:

- A **search index** is updated with the article's title and content, enabling keyword search via `kos search`.
- View adapters (tree, graph, table, timeline) are notified of the new entity and update their projections.

You can verify the search index is working:

```bash
kos search "knowledge management"
```

All derived data is disposable. If the search index is corrupted, rebuild it from canonical data:
